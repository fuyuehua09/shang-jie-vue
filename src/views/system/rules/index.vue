<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="rulesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="结算周期" align="center" prop="id" v-if="false"/>
      ...
      <el-table-column label="结算周期" align="center" prop="settlementCycle" />
...
<script>
import { listRules, getRules, delRules, addRules, updateRules, exportRules } from "@/api/system/rules";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除结算规则编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有结算规则数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
